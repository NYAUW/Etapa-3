...
public class EasyRandomClass {
	...
	public Cadastro cadastroRandomizer() {
		...
		cadastro.setEmail(faker.name().prefix().replace(" ", "").replace("'", "") + dominioEmail());
	...
	public Empresa empresaRandomizer() {
		...
		empresa.setProprietarios(faker.name().fullName());
	...
	public Endereco enderecoRandomizer() {
		...
		endereco.setBairro(
				(EnumBairros.values()[Double.valueOf(Math.random() * EnumBairros.values().length).intValue()])
						.getBairro());
	...
	public Funcionario funcionarioRandomizer() {
		...
		funcionario.setCargo((EnumCargoFuncionarios.values()[Double
				.valueOf(Math.random() * EnumCargoFuncionarios.values().length).intValue()]).getCargo());
		...
		funcionario.setSalario(
				new BigDecimal(fakeObject.regexify("[0-9]{4}").concat("." + fakeObject.regexify("[0-9]{2}"))));
		return funcionario;
	}
	...
	public static String geraCpf() {
		...
		return Arrays.toString(numeros).replace("[", "").replace(",", "").replace("]", "").replace(" ", "") + primeiro
				+ segundo;
	}
}
